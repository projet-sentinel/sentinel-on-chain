...
./create-user-stake.sh $FAKE_TREASURY
...
for (( i=1; i<=$AM_ADMINS; i++ ))
do 
    ./06addr-mint-tdat.sh $ORACLE_ADMIN$i $i $BAD 5
done
...
./07b-remove-threat-score.sh $ORACLE_ADMIN"1" 1 $FAKE_TREASURY 10
printfGreen "Sending the Ada to an other address than the one defined in threat database reference script datum, is not possible."
