...
raw="$TX_PATH/oracle-ref-sh-$INDEX.raw"
signed="$TX_PATH/oracle-ref-sh-$INDEX.signed"

UTXO_IN=$(get_address_biggest_lovelace $(cat $WALLET_PATH/$USER.addr))
UTXO_IN_REF_SCR=$(get_address_biggest_lovelace $(cat $WALLET_PATH/$2.addr))
...
submissionTime=$((nowSlotNumber + 90))
...
cabal run write-data Datum Controller ContractRefScriptDatum ThreatDatabase $DATUM_PATH/td-ref-script-dat.json $DATUM_PATH/threat-database-ref-dat-$INDEX.json
cabal run write-data Datum Controller ContractRefScriptDatum TDAT $DATUM_PATH/tdat-ref-script-dat.json $DATUM_PATH/tdat-ref-dat-$INDEX.json
...
cardano-cli transaction build \
    ...
    --tx-out-inline-datum-file $DATUM_PATH/td-ref-script-dat.json \
    ...
    --tx-out-inline-datum-file $DATUM_PATH/tdat-ref-script-dat.json \
